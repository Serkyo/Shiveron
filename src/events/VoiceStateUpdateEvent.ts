...
import {
	DISCORD_MAX_CHANNEL_NAME_LENGTH,
	DISCORD_SELECT_MENU_MAX_VALUES,
...
export default class VoiceStateUpdateEvent extends BaseEvent<'voiceStateUpdate'> {
	...
	public async execute(client: ShiveronClient, oldState: VoiceState, newState: VoiceState): Promise<void> {
		...
		if (channelId && oldState.channelId != newState.channelId) {
			await this.lock
				.acquire(channelId, async () => {
					try {
						const currentGuildOld = await client.guildSettingsService.createOrGetGuildSettings(
							oldState.guild.id,
						);
						const currentGuildNew = await client.guildSettingsService.createOrGetGuildSettings(
							newState.guild.id,
						);

						if (currentGuildNew.tempChannelId) {
							const t = (path: string, vars: Record<string, any> = {}) =>
								client.i18n.translate(currentGuildNew.lang, path, vars);
							await this.processVoiceChannelJoin(client, t, newState, currentGuildNew);
						}
						if (currentGuildOld.tempChannelId) {
							const t = (path: string, vars: Record<string, any> = {}) =>
								client.i18n.translate(currentGuildOld.lang, path, vars);
	 ...
	 */
	private async processVoiceChannelJoin(
		client: ShiveronClient,
		t: (path: string, vars?: Record<string, any>) => string,
		newState: VoiceState,
		currentGuildNew: GuildSettings,
	): Promise<void> {
		if (newState.member && newState.channelId) {
			if (newState.channelId == currentGuildNew.tempChannelId) {
				try {
					...
					client.logger.debug(
						`Created a new temporary voice channel ${newChannelId} in guild ${newState.guild.id} for user ${newState.member.id}`,
					);
				} catch (error) {
				...
				if (tempVoice && newState.member.id != tempVoice.ownerId) {
					...
					client.logger.debug(
						`Added user ${newState.member.id} to the successor list of channel ${newState.channelId} in guild ${newState.guild.id}`,
					);
				}
			}
		}
	 ...
	 */
	private async createTempChannel(
		client: ShiveronClient,
		t: (path: string, vars?: Record<string, any>) => string,
		newState: VoiceState,
	): Promise<string> {
		const { tempVoice, voiceACL, created } = await client.voiceService.createOrGetTempVoice(
			newState.guild.id,
			newState.member!,
		);
		...
		const newChannel = await newState.guild.channels.create({
			...
			permissionOverwrites: this.buildUserVoicePermissions(
				newState.member!.id,
		...
		const { menuText, menuEmbed, menuRow } = await this.createChannelControlMessage(
			t,
			newState.member!,
			newState.guild,
			created,
	 ...
	 */
	private async createChannelControlMessage(
		t: (path: string, vars?: Record<string, any>) => string,
		owner: GuildMember,
		guild: Guild,
		firstVoiceChannel: boolean,
		tempVoice: TempVoice,
		voiceACL: VoiceACL[],
		newChannel: VoiceChannel,
	): Promise<{ menuText: string; menuEmbed: EmbedBuilder; menuRow: ActionRowBuilder<StringSelectMenuBuilder> }> {
		const menuText = firstVoiceChannel ? t('temp_voice.control_message.first_channel', { user: owner }) : '';

		const soundBoardEnabledTemporarily = newChannel
			.permissionsFor(guild.roles.everyone)
			.has(PermissionFlagsBits.UseSoundboard);
		let soundboardStatus;
		if (tempVoice.soundBoardEnabled == soundBoardEnabledTemporarily) {
			soundboardStatus = soundBoardEnabledTemporarily
				? t('temp_voice.control_message.enabled')
				: t('temp_voice.control_message.disabled');
		} else {
			soundboardStatus = soundBoardEnabledTemporarily
				? t('temp_voice.control_message.enabled_temp')
				: t('temp_voice.control_message.disabled_temp');
		}

		const streamsEnabledTemporarily = newChannel
			.permissionsFor(guild.roles.everyone)
			.has(PermissionFlagsBits.Stream);
		let streamsStatus;
		if (tempVoice.streamsEnabled == streamsEnabledTemporarily) {
			streamsStatus = streamsEnabledTemporarily
				? t('temp_voice.control_message.enabled')
				: t('temp_voice.control_message.disabled');
		} else {
			streamsStatus = streamsEnabledTemporarily
		...
		const activitiesEnabledTemporarily = newChannel
			.permissionsFor(guild.roles.everyone)
			.has(PermissionFlagsBits.UseEmbeddedActivities);
		let activitiesStatus;
		if (tempVoice.activitiesEnabled == activitiesEnabledTemporarily) {
			activitiesStatus = activitiesEnabledTemporarily
				? t('temp_voice.control_message.enabled')
				: t('temp_voice.control_message.disabled');
		} else {
			activitiesStatus = activitiesEnabledTemporarily
		...
		const privateChannelTemporarily =
			!newChannel.permissionsFor(guild.roles.everyone).has(PermissionFlagsBits.ViewChannel) &&
			!newChannel.permissionsFor(guild.roles.everyone).has(PermissionFlagsBits.Connect);
		let privateChannelStatus;
		if (tempVoice.privateChannel == privateChannelTemporarily) {
			privateChannelStatus = privateChannelTemporarily
				? t('temp_voice.control_message.private')
				: t('temp_voice.control_message.public');
		} else {
			privateChannelStatus = privateChannelTemporarily
				? t('temp_voice.control_message.private_temp')
				: t('temp_voice.control_message.public_temp');
		}
		...
		const menuEmbed = new EmbedBuilder()
			...
			.setFooter({
				text: t('temp_voice.control_message.embed.footer', { user: owner.displayName }),
		...
		const menuRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menuSelect);
	 ...
	 */
	private async attachControlCollector(
		client: ShiveronClient,
		t: (path: string, vars?: Record<string, any>) => string,
		message: Message,
		channelOwner: GuildMember,
		newChannel: VoiceChannel,
	): Promise<void> {
	 ...
	 */
	private async createAutoMessageDeletion(
		client: ShiveronClient,
			...
			messageCollector.on('collect', async () => {
				...
				if (sortedMessages.size > messagesToKeep) {
					const toDelete = new Collection(
						sortedMessages.first(sortedMessages.size - messagesToKeep).map((m) => [m.id, m]),
					);

					const [bulkDeletableMessages, notBulkDeletableMessages] = toDelete.partition(
	 ...
	 */
	private async createSetAsDefaultQuestion(
		t: (path: string, vars?: Record<string, any>) => string,
		message: Message,
		targetId: string,
	): Promise<boolean> {
		const enable = new ButtonBuilder().setCustomId('enable').setEmoji('👍').setStyle(ButtonStyle.Success);
		const disable = new ButtonBuilder().setCustomId('disable').setEmoji('👎').setStyle(ButtonStyle.Danger);
		const buttonsRow = new ActionRowBuilder<ButtonBuilder>().addComponents([enable, disable]);
		...
		if (channel instanceof VoiceChannel) {
			...
			if (collectedMessages.size == 0) {
				...
				if (answer.content.length >= DISCORD_MAX_CHANNEL_NAME_LENGTH) {
					...
					const setAsDefaultQuestion = await answer.reply({
						content: t('temp_voice.process.name.success', { name: answer.content }),
					});

					const answerSetAsDefault = await this.createSetAsDefaultQuestion(
						t,
	 ...
	 */
	private async processSoundboardToggle(
		client: ShiveronClient,
		interaction: StringSelectMenuInteraction,
		t: (path: string, vars?: Record<string, any>) => string,
		channelOwnerId: string,
		...
		if (channel instanceof VoiceChannel) {
			const soundBoardEnabled = channel
			...
			const setAsDefaultQuestion = await interaction.editReply({
				content: t(
					!soundBoardEnabled
						? 'temp_voice.process.soundboards.success_enable'
						: 'temp_voice.process.soundboards.success_disable',
					{ channel },
				),
			});
	 ...
	 * @param channelOwnerId - The Discord user ID of the channel owner.
	 */
	private async processStreamToggle(
		client: ShiveronClient,
		interaction: StringSelectMenuInteraction,
		t: (path: string, vars?: Record<string, any>) => string,
		channelOwnerId: string,
		...
		if (channel instanceof VoiceChannel) {
			...
			const setAsDefaultQuestion = await interaction.editReply({
				content: t(
					!streamsEnabled
						? 'temp_voice.process.streams.success_enable'
						: 'temp_voice.process.streams.success_disable',
					{ channel },
				),
			});
	 ...
	 * @param channelOwnerId - The Discord user ID of the channel owner.
	 */
	private async processActivitiesToggle(
		client: ShiveronClient,
		interaction: StringSelectMenuInteraction,
		t: (path: string, vars?: Record<string, any>) => string,
		channelOwnerId: string,
		...
		if (channel instanceof VoiceChannel) {
			const activitiesEnabled = channel
				.permissionsFor(channel.guild.roles.everyone)
				.has(PermissionFlagsBits.UseEmbeddedActivities);
			...
			const setAsDefaultQuestion = await interaction.editReply({
				content: t(
					!activitiesEnabled
						? 'temp_voice.process.activities.success_enable'
						: 'temp_voice.process.activities.success_disable',
					{ channel },
				),
			});
	 ...
	 * @param channelOwnerId - The Discord user ID of the channel owner.
	 */
	private async processPrivateChannel(
		client: ShiveronClient,
		interaction: StringSelectMenuInteraction,
		t: (path: string, vars?: Record<string, any>) => string,
		channelOwnerId: string,
		...
		if (channel instanceof VoiceChannel) {
			const isPrivateChannel =
				...
				!channel.permissionsFor(channel.guild.roles.everyone).has(PermissionFlagsBits.Connect);
			...
			const setAsDefaultQuestion = await interaction.editReply({
				content: t(
					!isPrivateChannel
						? 'temp_voice.process.availability.success_private'
						: 'temp_voice.process.availability.success_public',
					{ channel },
				),
			});
	...
	private async processMessageDeletion(
		...
		if (channel instanceof VoiceChannel) {
			const amountMessage = await interaction.editReply({
				content: t('temp_voice.process.messages_kept.query_amount'),
			});
			...
			if (collectedMessages.size == 0) {
				...
				if (answer.content == 'none') {
					...
					amountMessage.reply({
						content: t('temp_voice.process.messages_kept.success', { amount: answer.content }),
					});
				} else {
	...
	private async processACL(
		...
		if (channel instanceof VoiceChannel) {
			...
			const rowSelection = new ActionRowBuilder<UserSelectMenuBuilder>().addComponents(memberSelect);

			const selectionMessage = await interaction.editReply({
				content: t(
					blacklist ? 'temp_voice.process.blacklist.query_users' : 'temp_voice.process.whitelist.query_users',
					{ channel },
				),
				components: [rowSelection],
			});

			const selectedUsers = (await awaitAuthorizedComponentInteraction(
				selectionMessage,
				channelOwnerId,
			...
			if (!selectedUsers) {
				...
				const voiceACL = await client.voiceService.getVoiceACLForTempVoice(
					interaction.guildId!,
					channelOwnerId,
				);
				...
				for (const userId of selectedUsers.values) {
					...
					if (blacklist) {
						if (isOnTargetList) {
							...
							client.voiceService.createOrUpdateVoiceACL(
								channel.guild.id,
								channelOwnerId,
								user.id,
								false,
	 ...
	 */
	private async processVoiceChannelLeave(
		client: ShiveronClient,
		t: (path: string, vars?: Record<string, any>) => string,
		oldState: VoiceState,
		currentGuildOld: GuildSettings,
	): Promise<void> {
		if (oldState.member && oldState.channel instanceof VoiceChannel) {
			...
			if (tempVoice) {
				if (oldState.channel.members.size == 0) {
					try {
						this.deleteTempChannel(
							client,
							oldState.guild.id,
							oldState.guild.channels,
							oldState.member.id,
							oldState.channel,
							currentGuildOld.tempChannelId!,
						);
						client.logger.debug(
							`Deleted a temporary voice channel ${oldState.channel.id} in guild ${oldState.guild.id}`,
						);
					} catch (error) {
					...
					try {
						const newOwnerId = await this.changeTempChannelOwner(
							client,
							t,
							oldState.guild,
							tempVoice.ownerId,
							tempVoice.successorIds,
							currentGuildOld.tempChannelId!,
							oldState.channel,
							tempVoice.channelControlMessageId!,
						);
						client.logger.debug(
							`Transferred ownership of a temporary voice channel ${oldState.channel.id} in guild ${oldState.guild.id} from ${tempVoice.ownerId} to ${newOwnerId}`,
						);
					} catch (error) {
						client.logger.error(
							'An error occured while transfering ownership of a temporary voice channel',
						);
						throw error;
					}
				} else {
					...
					if (index != -1) {
						...
						client.logger.debug(
							`Removed user ${oldState.member.id} from the successor list of channel ${oldState.channelId} in guild ${oldState.guild.id}`,
						);
					}
				}
			}
	 ...
	 */
	private async deleteTempChannel(
		client: ShiveronClient,
		guildId: string,
		channels: GuildChannelManager,
		ownerId: string,
		tempChannel: VoiceChannel,
		createTempChannelId: string,
	): Promise<void> {
		tempChannel.delete();
	 ...
	 */
	private async changeTempChannelOwner(
		client: ShiveronClient,
		t: (path: string, vars?: Record<string, any>) => string,
		guild: Guild,
		oldOwnerId: string,
		...
		await channel.permissionOverwrites.set(
			this.buildUserVoicePermissions(newOwnerId, guild.roles.everyone.id, tempVoice, voiceACL),
		);
	 ...
	 */
	private async cleanupTempChannel(
		client: ShiveronClient,
		guildId: string,
		ownerId: string,
		channels: GuildChannelManager,
	 ...
	 */
	private buildUserVoicePermissions(
		ownerId: string,
		everyoneId: string,
		tempVoice: TempVoice,
		voiceACL: VoiceACL[],
	 ...
	 */
	private async refreshChannelControls(
		client: ShiveronClient,
		t: (path: string, vars?: Record<string, any>) => string,
		interaction: StringSelectMenuInteraction,
		channelOwner: GuildMember,
